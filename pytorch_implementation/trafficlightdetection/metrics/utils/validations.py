...
from trafficlightdetection.metrics.utils.enumerators import (
    BBFormat,
    BBType,
    FileFormat,
    CoordinatesType,
)
from trafficlightdetection.metrics.utils.general_utils import get_files_recursively
...
def is_pascal_format(file_path):
    ...
    """
    return is_xml(file_path) and xml_contains_tags(
        file_path, ["annotation", "./size/width", "./size/height"]
    )
...
def is_cvat_format(file_path):
    ...
    """
    return is_xml(file_path) and xml_contains_tags(
        file_path, ["annotations", "./image/box"]
    )


def is_specific_text_format(
    file_path, type_coordinates=CoordinatesType.ABSOLUTE, bb_type=BBType.GROUND_TRUTH
):
...
def is_absolute_text_format(file_path, num_blocks=[6, 5], blocks_abs_values=[4]):
    ...
    if not is_empty_file(file_path):
        return all_lines_have_blocks(
            file_path, num_blocks=num_blocks
        ) and all_blocks_have_absolute_values(
...
def is_relative_text_format(file_path, num_blocks=[6, 5], blocks_rel_values=[4]):
    ...
    if not is_empty_file(file_path):
        return all_lines_have_blocks(
            file_path, num_blocks=num_blocks
        ) and all_blocks_have_relative_values(
...
def is_yolo_format(file_path, bb_types=[BBType.GROUND_TRUTH, BBType.DETECTED]):
    ...
    return (
        is_text(file_path)
        and all_lines_have_blocks(file_path, num_blocks=num_blocks)
        and all_blocks_have_relative_values(file_path, blocks_rel_values=[4])
    )
